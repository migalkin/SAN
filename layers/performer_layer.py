"""
    Thanks LucidRains! https://github.com/lucidrains/performer-pytorch/blob/main/performer_pytorch/performer_pytorch.py
"""
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_e = nn.Linear(dim, inner_dim, bias = qkv_bias)  # added edge transformation

        if self.full_graph:
            self.to_q2 = nn.Linear(dim, inner_dim, bias = qkv_bias)
            self.to_k2 = nn.Linear(dim, inner_dim, bias = qkv_bias)
            self.to_e2 = nn.Linear(dim, inner_dim, bias = qkv_bias)

        # for full graph
        self.to_out = nn.Linear(inner_dim, dim, bias = attn_out_bias)
        self.dropout = nn.Dropout(dropout)

    def forward(self, g, x, e, pos_emb = None, context = None, mask = None, context_mask = None, **kwargs):


        # Get ids for real and fake edges
        if self.full_graph:
        ...
        # transformation for real edges
        q, k, v, e = self.to_q(x), self.to_k(context), self.to_v(context), self.to_e(e)

        # TODO scatter aggregation of real edge features by source node
        # agg_e = torch.scatter(e[real_ids], index=TODO, 0)
        agg_e = torch.zeros_like(k, dtype=k.dtype, device=k.device)
        # output: N x dim
        k += agg_e

        # transformation for fake edges
        if self.full_graph:
            q2, k2, e2 = self.to_q2(x), self.to_k2(x), self.to_e2(e)

            # TODO scatter aggregation of fake edge features by source node
            # Features of fake edges are initialized as zero -> any linear transformation will also produce 0
            # Do we need it in this case?
            # dummy_fake_e = torch.scatter(e[fake_ids], index=TODO, 0)
            dummy_fake_e = torch.zeros_like(k2, dtype=k2.dtype, device=k2.device)
            k2 += dummy_fake_e

            q2, k2 = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), (q2, k2))
            (q2, lq2), (k2, lk2) = map(lambda t: (t[:, :gh], t[:, gh:]), (q2, k2))
        ...
        if not empty(q):
            ...
            if self.full_graph:
                out2 = self.fast_attention(q2, k2, v)

                # weight with lambda
                out = out / (1 + self.gamma) + self.gamma * out2 / (1 + self.gamma)
        ...
        if not empty(lq):
            ...
            if self.full_graph:
                out2 = self.local_attn(lq2, lk2, lv, input_mask = mask)
                # weight with lambda
                out = out / (1 + self.gamma) + self.gamma * out2 / (1 + self.gamma)
...
class Performer(nn.Module):
    def __init__(
        ...
        heads,
        dim_head,
        gamma, # for weighting attention on real/fake edges from SAN
        local_attn_heads = 0,
        local_window_size = 256,
        causal = False,
        ff_mult = 4,
